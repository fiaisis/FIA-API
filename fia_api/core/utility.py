...
GITHUB_PACKAGES_CACHE_TTL_SECONDS = int(os.environ.get("GITHUB_PACKAGES_CACHE_TTL_SECONDS", "300"))


def forbid_path_characters(func: FuncT) -> FuncT:
    """Decorator that prevents path characters {/, ., \\} from a functions args
    by raising UnsafePathError."""
...
def filter_script_for_tokens(script: str) -> str:
    """Filters out lines that contain tokens i.e. 'ghp_' and
    'network.github.api_token' from the script, by cutting that line.
...
def safe_check_filepath(filepath: Path, base_path: Path) -> None:
    """Check to ensure the path does contain the base path and that it does not
    ...
    try:
        ...
        if not filepath.is_relative_to(base_path):
            raise AuthError("Invalid path being accessed.")
...
def get_packages(org: str, image_name: str) -> Any:
    ...
    cache_key = f"fia_api:github:packages:{org}:{image_name}"
    ...
    cache_set_json(cache_key, payload, GITHUB_PACKAGES_CACHE_TTL_SECONDS)
    return payload
...
def find_file_experiment_number(ceph_dir: str, experiment_number: int, filename: str) -> Path | None:
    ...
    :param experiment_number: experiment_number of the user who made the
        file and dir path
...
def find_file_user_number(ceph_dir: str, user_number: int, filename: str) -> Path | None:
    ...
    :param ceph_dir: base path of the path
    :param user_number: user number of the user who made the file and
        dir path
    :param filename: filename to find
    :return: Full path to the filename or None
    """
...
def _safe_find_file_in_dir(dir_path: Path, base_path: str, filename: str) -> Path | None:
    """Check that the directory path is safe and then search for filename in
    that directory and sub directories.

    :param dir_path: Path to check is safe and search in side of
    :param base_path: the base directory of the path often just the
        /ceph dir on runners
    :param filename: filename to find
    ...
    try:
        ...
        raise AuthError("Invalid path being accessed.") from None
...
def request_path_check(path: Path, base_dir: str) -> Path:
    ...
    """
    if path is None:
        raise MissingRecordError("Invalid or nonexistent path")
    # Remove the base_dir
