...
class PearlAutomation:
    ...
    def authenticate(self):
        ...
        try:
            response = requests.post(f"{self.auth_url}/login", json={"username": self.username, "password": self.password}, timeout=30)
    ...
    def monitor_job(self, job_id, poll_interval=5):
        ...
        while True:
            ...
            if state == State.SUCCESSFUL.value:
                ...
                return job_data
            elif state in [State.ERROR.value, State.UNSUCCESSFUL.value]:
    ...
    def download_results(self, job_id, outputs):
        ...
        for filename in filenames:
            ...
            response = requests.get(f"{self.fia_url}/job/{job_id}/filename/{filename}", headers=self.get_headers(), timeout=30, stream=True)
            response.raise_for_status()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--auth-url", default=os.environ.get("AUTH_API_URL", "http://localhost:8001"), help="Auth API URL")
    ...
    parser.add_argument("--output-dir", default=os.environ.get("OUTPUT_DIRECTORY", "./output"), help="Output directory for results")
    parser.add_argument("--runner", default=os.environ.get("MANTID_RUNNER_IMAGE"), help="Specific Mantid runner image to use")
    
    args = parser.parse_args()
    
    if not args.username or not args.password:
        logger.error("Username and password must be provided via arguments or environment variables (PEARL_USERNAME, PEARL_PASSWORD)")
        sys.exit(1)
        
    automation = PearlAutomation(args.fia_url, args.auth_url, args.username, args.password, args.output_dir, args.runner)
    automation.run()
