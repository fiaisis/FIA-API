...
@patch("fia_api.core.auth.tokens.requests.post")
@patch("fia_api.core.auth.experiments.requests.get")  # HERE
...
def test_3(mock_post, mock_get_experiment_numbers_for_user_number):
    ...
    assert response_as_user_true.status_code == HTTPStatus.OK

    response_as_user_false = client.get(
        "/instrument/mari/jobs?&as_user=false", headers={"Authorization": f"Bearer {USER_TOKEN}"}
    )
    assert response_as_user_false.status_code == HTTPStatus.OK
