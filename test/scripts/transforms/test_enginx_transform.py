...
def script():
    ...
    """
    return PreScript(
        value="""# Mantid Repository : https://github.com/mantidproject/mantid
#
...
def reduction_with_prefix():
    ...
    mock.inputs = {
        ...
        "focus_runs": ["ENGINX765432", "876543"],
...
def test_enginx_transform_apply(script, reduction):
    ...
    # Check that all instances of vanadium_run, focus_runs, and ceria_run are updated
    for index, line in enumerate(updated_lines):
        if (
            ('vanadium_run="' in line and 'vanadium_run="ENGINX654321"' not in line)
            or (
                'focus_runs=["' in line
                and 'focus_runs=["ENGINX765432", "ENGINX876543"]' not in line
                and 'focus_runs=["ENGINX299080"]' not in line
                and 'focus_runs=["ENGINX305761"]' not in line
            )
...
def test_enginx_transform_with_string_focus_runs(script):
    ...
    # Check that all instances of focus_runs are updated correctly
    focus_runs_updated = False
    for line in updated_lines:
        if "focus_runs=" in line and "ENGINX765432" in line:
            focus_runs_updated = True
            break

    assert focus_runs_updated, "No focus_runs parameter was updated to include 'ENGINX765432'"
